...
def main(args):
    ...
    for seed in seed_list:
        ...
        if os.path.isdir(experiment_folder + 'generations/'):
            print('removing generations')
            shutil.rmtree(experiment_folder + 'generations/')
        ...
        if hparams['y_condition']:
            ...
            def sample_YD(model, idx, batch_size=args.batch_size, temp=args.temperature):
                with torch.no_grad():
                    if hparams['yd_condition']:
                        ...
                        images0 = model(z=None, y_onehot=y_0, temperature=temp, reverse=True, batch_size=batch_size)
                        images1 = model(z=None, y_onehot=y_1, temperature=temp, reverse=True, batch_size=batch_size)
                return images0, images1

            def sample_DD(model, idx, batch_size=args.batch_size, temp=args.temperature):
                with torch.no_grad():
                    if hparams['yd_condition']:
            ...
            for idx, dom in enumerate(
                ...
                print("original sample sizes", images0.size(), images1.size())
                if args.filter_black:
                    images0 = filter_black(images0, device=device)
                    images1 = filter_black(images1, device=device)
                print("Final sample sizes", images0.size(), images1.size())
                ...
                images_concat0 = torchvision.utils.make_grid(images0[:64, :, :, :], nrow=int(64 ** 0.5), padding=2,
                                                             pad_value=255)
                torchvision.utils.save_image(images_concat0, experiment_folder + dom + str(args.filter_black) + str(args.temperature)+ '_uninfected.png')
                images_concat1 = torchvision.utils.make_grid(images1[:64,:,:,:], nrow=int(64** 0.5), padding=2, pad_value=255)
                torchvision.utils.save_image(images_concat1, experiment_folder + dom + str(args.filter_black)  + str(args.temperature)+ '_parasitized.png')
        ...
        # os.makedirs('d_unconditioned/generations/' + experiment_folder + 'Uninfected', exist_ok=True)
            ...
            #
def filter_black(image_stack, device, black_thresh=50, proportion=0.50, sample_size=100):
    idx_list=[]
    for idx in range(image_stack.size(0)):
        im = transforms.ToPILImage()(image_stack.cpu()[idx])
        #im = Image.open(image_stack[idx]) 
...
#             print(pixel)
        ...
        if (nblack / float(n)) > proportion:
#             print("mostly black")
            idx_list.append(idx)
        if len(idx_list)>=sample_size:
            break
    return image_stack[idx_list].to(device)
if __name__ == '__main__':
    ...
    parser.add_argument('--filter_black', action='store_true')
